...
### 1. Using `initialQuery` and `filters`
...
### 2. Adding the `onDownload` callback
  ...
  return (
    <AuditLogViewer
      ...
      onDownload={async (body) => {
        // This function calls your server's /download_results
        // endpoint and returns a URL or file data
...
### 3. Enabling Verification with `verificationOptions`
...
Remember to explore [our GitHub repository](https://github.com/pangeacyber/pangea-javascript/tree/main/packages/react-mui-audit-log-viewer) for full Storybook examples and local testing instructions. If you have questions, check out our [Pangea Docs](https://pangea.cloud/docs/) or join our community on [Discourse](https://l.pangea.cloud/Jd4wlGs).

If you have questions or suggestions for improvement please reach out, we are happy to make updates and improve the component for use by customers.
