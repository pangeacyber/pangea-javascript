    ...
    while (Date.now() < MAX_TIME) {
      ...
      if (lockObj === null) {
        ...
        if (lockObjPostDelay !== null) {
          ...
          if (
            parsedLockObjPostDelay.id === this.id &&
            parsedLockObjPostDelay.iat === iat
          ) {
    ...
    for (let i = 0; i < KEYS.length; i++) {
      ...
      if (LOCK_KEY.includes(LOCK_STORAGE_KEY)) {
        ...
        if (lockObj !== null) {
          let parsedlockObj = JSON.parse(lockObj);
          if (
            (parsedlockObj.timeRefreshed === undefined &&
              parsedlockObj.timeAcquired < MIN_ALLOWED_TIME) ||
            (parsedlockObj.timeRefreshed !== undefined &&
              parsedlockObj.timeRefreshed < MIN_ALLOWED_TIME)
          ) {
