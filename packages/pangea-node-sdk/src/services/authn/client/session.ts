...
export default class ClientSession extends BaseService {
  ...
  invalidate(token: string, sessionID: string): Promise<PangeaResponse<{}>> {
    ...
    return this.post("v1/client/session/invalidate", data);
  }
  ...
  list(
    ...
    return this.post("v1/client/session/list", data);
  }
  ...
  logout(token: string): Promise<PangeaResponse<{}>> {
    return this.post("v1/client/session/logout", { token });
  }
  ...
  refresh(
    ...
    return this.post("v1/client/session/refresh", data);
  }
}
